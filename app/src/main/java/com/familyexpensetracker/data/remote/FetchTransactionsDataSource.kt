...
class FetchTransactionsDataSource(private val service: Sheets) {
    ...
    suspend fun fetch(
        ...
        try {
            Log.d(TAG, "Fetching from Spreadsheet: $spreadsheetId, Range: $range")
            ...
            Log.d(TAG, "Fetched ${values.size} rows (including header)")
    ...
    private fun convertToTransactions(rows: Sequence<List<Any>>): List<Transaction> =
        rows.map { rowMapper.mapRow(it) }.toList()

    companion object {
        private val TAG = FetchTransactionsDataSource::class.simpleName!!
    }
}
